...
export default function ProdutoPage() {
    ...
    useEffect(() => {
        async function fetchProduto() {
            try {
                const response = await axios.get<ProdutoType>(`http://localhost:3001/produtosDestaque/${id}`);
            ...
            <div className="flex flex-col lg:flex-row gap-10 bg-white shadow-md rounded-2xl p-6 md:p-10">
                ...
                <div className="flex-1 flex flex-col justify-between">
                    ...
                    <div className="mt-10 flex flex-col sm:flex-row gap-4">
                        <button className="flex gap-4 bg-green-600 hover:bg-green-700 text-white px-6 py-3 rounded-xl font-semibold shadow-lg transition cursor-pointer">
                            faça seu pedido pelo WhatsApp <FaWhatsapp size={28} />
                        </button>

                        <Link href="/">
                            <button className="border border-gray-400 hover:bg-gray-100 text-gray-800 px-6 py-3 rounded-xl font-semibold transition shadow-lg">
                                Voltar
                            </button>
                        </Link>
                    </div>
